...
パッヘルベルのカノン（3トラック・カノン形式のサンプル）:

```bash
# 通奏低音 + 2声のカノン（2小節遅れで同じ旋律が入る）を同時に鳴らす。2声目だけ波形を変えて聞き分けやすくする
python sfx_generator.py --input-file scores/canon_in_d_bass.mml --track-file scores/canon_in_d_round1.mml --track-file scores/canon_in_d_round2.mml --track-style sine --track-style triangle --style sine -o canon_in_d_sample
```

`scores/canon_in_d_bass.mml`（通奏低音）、`scores/canon_in_d_round1.mml`（カノン1声目）、`scores/canon_in_d_round2.mml`（2小節遅れで入る2声目）の3ファイルを`--track-file`で重ねています。通奏低音はD-A-Bm-F#m-G-D-G-Aの進行で、同じ旋律をずらして重ねる「カノン」の構造をそのまま表現したサンプルです。`--track-style`で2声目（round2）だけ波形をtriangleに変え、1声目（round1, sine）と聞き分けやすくしています。バロック期（17世紀）の作品で著作権の問題はありません。
...
```bash
python sfx_generator.py --input "O4 L4 T120 C D E G" --style square -o melody.wav
```
...
MMLの音符直後の数字は、PPMCK / MCKと同じく**音長**を表します（`C4` = 四分音符のC）。省略時はLコマンドで指定したデフォルト音長を使います。オクターブは`O`コマンド、または相対オクターブ変更`>`（1つ上げる）/ `<`（1つ下げる）で指定します。

```text
PPMCK / MCK: c4 = 四分音符のC
現行OnGen:   C4 = 四分音符のC（オクターブはOコマンド / > / < で指定）
```

| コマンド | 意味 |
|---------|------|
| `C D E` | ノート（音名。音長省略時はLコマンドの値） |
...
旧バージョンでは音符直後の数字をオクターブとして解釈していましたが、現在は廃止されています。旧形式の楽譜は、オクターブ指定を`O` / `>` / `<`に書き換えてください。

上記のいずれにも一致しないトークンは、サイレントスキップではなく`ValueError`としてパーサーエラーになります。
...
python sfx_generator.py --input "O4 L1 A" --style sine --fm --fm-ratio 2 --fm-index 3 -o fm

# 2オペレーターFM音色プリセット
python sfx_generator.py --input "O4 L1 A" --fm-preset bell -o fm_bell
python sfx_generator.py --input "O4 L1 A" --fm-preset e-piano -o fm_e_piano
python sfx_generator.py --input "O2 L1 A" --fm-preset bass -o fm_bass

# ビブラート: 音程を周期的に揺らす
python sfx_generator.py --input "O4 L1 A" --style sine --lfo --lfo-target pitch --lfo-rate 5 --lfo-depth 0.02 -o lfo_pitch

# トレモロ: 音量を周期的に揺らす
python sfx_generator.py --input "O4 L1 A" --style sine --lfo --lfo-target volume --lfo-rate 5 --lfo-depth 0.8 -o lfo_volume

# PWM: 矩形波のデューティ比を周期的に揺らす
python sfx_generator.py --input "O4 L1 A" --style square --lfo --lfo-target duty --lfo-rate 2 --lfo-depth 0.5 -o lfo_duty

# MMLコマンドで FM + ビブラート
python sfx_generator.py --input "O4 L8 %12 *4 ~8 C E G" --lfo --style sine -o fm_lfo
```
...
```bash
# 短く減衰する打撃音向けノイズ
python sfx_generator.py --input "O4 L8 C" --style noise --attack 0.001 --decay 0.08 --sustain 0 --release 0.05 -o noise_hit

# 柔らかいピンクノイズ
python sfx_generator.py --input "O4 L1 C" --style noise --noise-color pink --filter lowpass --cutoff 3000 -o noise_pink

# 低域の強いブラウンノイズ
python sfx_generator.py --input "O4 L1 C" --style noise --noise-color brown --filter lowpass --cutoff 800 -o noise_brown

# 高域だけを残したホワイトノイズ
python sfx_generator.py --input "O4 L1 C" --style noise --noise-color white --filter highpass --cutoff 5000 -o noise_highpass
```
...
```bash
# MML でサンプルと合成音を混在
python sfx_generator.py --input "T120 L8 W(kick.wav) R4 C" --sample-root samples -o mixed.wav

# 生成した音の上にサンプルを重ねる
# 形式: ファイルパス[:開始秒[:ゲイン]]
python sfx_generator.py --input "O4 L4 C D E" --overlay-sample "samples/kick.wav:0:0.6" -o overlay.wav
```
...
各トラックには `--style` などの全体設定が共通で適用されます。音色を変えたい場合は MML の `@1`〜`@7`（音色プリセット）をトラックごとに指定するか、`--track-style`（基本波形のみ）を使ってください。

```bash
# メイントラックは矩形波、追加トラックはサイン波で鳴らす
python sfx_generator.py --input "O4 L4 T120 C D E F" --style square --track "O3 L4 T120 C E G C" --track-style sine -o duet
```

`--track-style` は `--track` / `--track-file` を指定した順に対応し、波形（square/sawtooth/triangle/sine/noise）のみを上書きします。指定しないトラックには `--style` の値が使われます。ADSR・FM・LFOなどは全トラック共通です。
...
python sfx_generator.py --input "O4 L4 C D E" -o jump --output-format all --bitrate 192
...
## Rulesync の導入（別プロジェクトに構成を持ち込む場合）

本プロジェクトの `.rulesync/` 構成を別プロジェクトへ持ち込む場合、ルール最新化は `corepack pnpm dlx rulesync generate` をベースに行います。corepack 経由で pnpm を使うため、rulesync のグローバルインストールは不要です。

```bash
# corepack が未有効な場合は一度だけ実行
corepack enable
...
詳細な導入フローは [dna_kernel / Rulesync](#dna_kernel--rulesync) と `docs/dna-kernel/onboarding.md` を参照してください。
